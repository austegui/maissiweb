...
<task type="auto">
  ...
  <action>
    ...
    **3. Update `src/hooks/use-auto-polling.ts` (REQUIRED):**

    The `useAutoPolling` hook stores `interval` in a ref but does NOT react when the prop value changes after mount. This must be fixed for the 5s/10s fallback to work.

    Add a `useEffect` that syncs the refs when the `interval` prop changes:
    ```typescript
    useEffect(() => {
    ...
    ```
    This ensures that when `realtimeConnected` toggles (causing `pollingInterval` to change from 10000 to 5000 or back), the polling timer picks up the new interval on the next cycle.

    **Verify this fix**: Confirm that `baseIntervalRef` and `currentIntervalRef` are updated by the new `useEffect`. Without this, the polling interval never switches to 5s when Realtime disconnects, breaking REALTIME-02/03 fallback behavior.
  </action>
  <verify>
    ...
    - `use-auto-polling.ts` contains a `useEffect` with `[interval]` dependency that updates `baseIntervalRef.current` and `currentIntervalRef.current`
  </verify>
